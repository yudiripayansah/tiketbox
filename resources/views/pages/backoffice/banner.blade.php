...
@section('screen')
  <section id="bBanners" class="br-tl-10 br-tr-10 overflow-hidden backoffice">
    ...
    methods: {
      ...
      async doSave() {
        ...
        try {
          ...
          if(payload.id) {
            req = await tiketboxApi.updateBanner(payload,token)
          } else {
            req = await tiketboxApi.createBanner(payload,token)
          }
      ...
      async doUpdate(id) {
        try {
          if(id){
            ...
            let token = 'abcdreUYBH&^*VHGY^&GY'
              let req = await tiketboxApi.getBanner(payload,token)
              let { status, msg, data} = req.data
              if(status){
                this.form.data = data
              } else {
        ...
        if(state){
          try {
            if(id){
              ...
              let token = 'abcdreUYBH&^*VHGY^&GY'
                let req = await tiketboxApi.deleteBanner(payload,token)
                let { status, msg, data} = req.data
                if(status){
                  this.notify('success','Success',msg)
                  this.doGet()
